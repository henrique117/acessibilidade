...
<head>
    ...
    <style>
        body { font-family: 'Helvetica', 'Arial', sans-serif; color: #333; font-size: 11pt; line-height: 1.5; margin: 0; padding: 0; }
        ...
        .stat-card { flex: 1; background: #f8f9fa; border: 1px solid #e9ecef; padding: 20px; text-align: center; border-radius: 8px; }
        ...
        .ranking-item { display: flex; justify-content: space-between; padding: 15px 20px; border-bottom: 1px solid #eee; align-items: center; }
        .ranking-item:last-child { border-bottom: none; }
        .rank-pos { background: #e74c3c; color: #fff; width: 30px; height: 30px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; margin-right: 15px; flex-shrink: 0; }
        ...
        .page-header { background: #2c3e50; color: #fff; padding: 15px; font-size: 14pt; margin-top: 40px; margin-bottom: 20px; border-left: 8px solid #3498db; }
        ...
        .erro-top { display: flex; justify-content: space-between; border-bottom: 1px solid #eee; padding-bottom: 10px; margin-bottom: 10px; align-items: center;}
        ...
        .badge-criterio { display: inline-block; background: #e3f2fd; border: 1px solid #bbdefb; color: #1565c0; padding: 3px 8px; border-radius: 12px; font-size: 0.85em; font-weight: bold; margin-right: 5px; margin-bottom: 5px; }
        .container-criterios { margin-bottom: 15px; padding: 10px; background-color: #f8f9fa; border-left: 4px solid #b0bec5; border-radius: 4px; }
...
<body>
    ...
    <div id="visao-geral">
        ...
        <table class="tabela-stats">
            ...
            <tbody>
                <tr><td><strong>Alta</strong></td><td>{{ $estatisticas['perc_Alta'] }}</td></tr>
                <tr><td><strong>Média</strong></td><td>{{ $estatisticas['perc_Media'] }}</td></tr>
                <tr><td><strong>Baixa</strong></td><td>{{ $estatisticas['perc_Baixa'] }}</td></tr>
            </tbody>
        </table>
                ...
                @foreach($dadosPagina['erros'] as $erro)
                    <div class="erro-card">
                        <div class="erro-top">
                            ...
                            @if($erro->criticidade)
                                <span class="badge-criticidade {{ $erro->criticidade == 'Alta' ? 'bg-alta' : ($erro->criticidade == 'Média' ? 'bg-media' : 'bg-baixa') }}">
                                    {{ $erro->criticidade }}
                                </span>
                            @endif
                        </div>
                        ...
                        @if(!empty($erro->comportamento_esperado))
                            <div class="comportamento-box">
                                <span class="label" style="margin-top: 0; color: #2c3e50;">Comportamento Esperado:</span>
                                <div style="font-style: italic; color: #444;">{!! $erro->comportamento_esperado !!}</div>
                            </div>
                        @endif
